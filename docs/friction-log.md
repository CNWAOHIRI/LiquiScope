...
## 2026-07-27 — Telegram alert relay: a real self-fetch KV inconsistency

### FL-017 — Cron Trigger self-fetch to the Worker's own route got a reproducible false-negative KV read
- **What:** Watch Mode's cron (`*/15 * * * *`) delivers alerts by POSTing to whatever `notify_webhook` a subscription registered. When that URL was this Worker's own `/telegram/relay/:token` endpoint, every real cron-tick delivery attempt failed with `webhook responded HTTP 404 ("unknown relay token")` — three consecutive ticks, ~30 minutes. At the same time, calling the *exact same URL* manually via external curl succeeded every single time (4/4), and direct KV inspection (`wrangler kv key get --remote`) confirmed the token→chatId mapping genuinely existed throughout. Added temporary per-subscription failure logging (host + error, not the full URL/token) to confirm this wasn't a network-level failure or routing miss — the self-fetch completed and reached the real handler; only `getChatIdForToken`'s KV read, specifically inside that self-fetch's execution, came back empty.
- **Root cause:** not fully diagnosed (Cloudflare doesn't expose enough visibility to prove it conclusively), but every symptom is consistent with a Cron Trigger self-fetch landing at a specific edge location whose local KV read-cache had a stale/negative cached result for that key, while all other request origins (my curl calls, from an entirely different network path) hit different edges with fresh data. Cloudflare KV's documented edge cache TTL (~60s) doesn't obviously explain persistence across 3 ticks (~30 min) unless negative-caching or per-colo propagation is a longer/different story than the general docs describe — this is inference, not confirmed platform behavior.
- **Workaround (not a workaround — an actual architecture fix):** rather than chase an unconfirmed root cause, removed the self-fetch entirely. `telegram/deliver.ts` now recognizes when a subscription's `notify_webhook` is this Worker's own relay URL and delivers the Telegram message via a direct in-process call (same KV binding, same invocation, no HTTP round-trip to itself) — `cron.ts`'s `deliverAlert` dispatches to this path automatically; every other webhook target is unaffected, still a normal `fetch()`. `POST /telegram/relay/:token` still exists and still works for any external caller that registers it as a `notify_webhook` from outside this Worker.
- **Severity:** medium — this was a real, reproducible production failure of a just-shipped feature (Telegram alerts silently never fired), caught within the same session via direct evidence-gathering (logging + independent KV/curl verification) rather than shipped un-investigated. Worth remembering for any FUTURE feature that has a Cloudflare Worker call its own public routes from inside a Cron Trigger — prefer an in-process call over a self-fetch on this platform until/unless the underlying behavior is better understood.
